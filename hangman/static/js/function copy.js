document.addEventListener("DOMContentLoaded", () => {
  ...
  let APalabra = "",
    ...
    palabrasUsadas = new Set(); // Para rastrear palabras ya usadas
  ...
  // Función para deshabilitar los selectores
  const bloquearSelectores = (estado) => {
  ...
  const proporcionarAyuda = () => {
    ...
    const botonLetra = ContenedorLetras.querySelector(
      `button[data-letra="${letraAleatoria}"]`
    );
    if (botonLetra) {
  ...
  const redirigirALogin = () => {
    location.reload();
    window.location.href = "/"; // Redirige a la ruta raíz donde está el login
  };

  // Agregar el evento click al botón salir
  if (btnSalir) {
    btnSalir.addEventListener("click", redirigirALogin);
  }
